...
import 'package:tumble/ui/main_app_widget/account_page/widgets/available_event_card_small.dart';
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<UserEventListCubit, UserEventListState>(
      listener: (context, state) {
        if (state.status == UserEventListStatus.LOADING && state.refreshSession) {
          BlocProvider.of<AuthCubit>(context).login();
          BlocProvider.of<UserEventListCubit>(context)
      ...
      Flexible(
        ...
        child: ListView(
          ...
          children: state.userEvents!.unregisteredEvents.isNotEmpty
              ? state.userEvents!.unregisteredEvents.map((e) => AvailableEventCardSmall(event: e)).toList()
              : state.userEvents!.registeredEvents.map((e) => AvailableEventCardSmall(event: e)).toList(),
        ),
      ),
    ],
  );
...
Widget _availableUserEventsDivider(BuildContext context, UserEventListState state) {
  return Row(
    children: [
      Padding(
        padding: const EdgeInsets.only(right: 12),
        child: Text(
          state.userEvents!.unregisteredEvents.isNotEmpty ? "Available exams" : "Your upcoming exams",
          style: TextStyle(
            color: Theme.of(context).colorScheme.onBackground,
      ...
      IconButton(
          ...
          splashRadius: 24,
          onPressed: () => BlocProvider.of<UserEventListCubit>(context).getUserEvents(
                BlocProvider.of<AuthCubit>(context).state.userSession!.sessionToken,
              ),
