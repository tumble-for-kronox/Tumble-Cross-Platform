...
class UserEventListCubit extends Cubit<UserEventListState> {
  UserEventListCubit() : super(UserEventListState(status: UserEventListStatus.LOADING));

  final UserRepository _userRepository = locator<UserRepository>();

  Future<void> getUserEvents(String sessionToken) async {
    emit(state.copyWith(status: UserEventListStatus.LOADING));
    ApiResponse userEventResponse = await _userRepository.getUserEvents(sessionToken);

    switch (userEventResponse.status) {
      case ApiStatus.REQUESTED:
        emit(state.copyWith(
            status: UserEventListStatus.LOADED, userEvents: userEventResponse.data!, refreshSession: false));
        break;
      case ApiStatus.UNAUTHORIZED:
        ...
        break;
      default:
        emit(state.copyWith(status: UserEventListStatus.ERROR, refreshSession: false));
    }
  }
}
