...
class NotificationService {
  ...
  Future<void> assignAllNotificationsWithNewDuration(
      Duration oldOffset, Duration newOffset) async {
    final List<String>? bookmarkedScheduleIds = _preferenceService.bookmarkIds;
    if (bookmarkedScheduleIds == null) return;

    List<NotificationModel> currentNotifications = await _awesomeNotifications
        .getAllNotificationsFromChannels(bookmarkedScheduleIds);
    ...
    List<NotificationModel> newNotifications = await _awesomeNotifications
        .getAllNotificationsFromChannels(bookmarkedScheduleIds);
    ...
    for (NotificationModel newNotification in newNotifications) {
      ...
      Map<String, dynamic> newNotificationSchedule =
          newNotification.schedule!.toMap();

      NotificationModel oldNotification = currentNotifications.firstWhere(
          (notification) =>
              notification.content!.id! == newNotification.content!.id!);
      Map<String, dynamic> oldNotificationSchedule =
          oldNotification.schedule!.toMap();
  ...
  Future<bool> initialize() async {
    ...
    return await getIt<AwesomeNotifications>().initialize(_defaultIcon, [
      ...
      _notificationServiceBuilder.buildNotificationChannel(
        ...
        channelDescription:
            "A notification channel for the school resources and user bookings systems",
      ),
    ]);
  }
  ...
  Future<bool> allowedNotifications() async =>
      _awesomeNotifications.isNotificationAllowed();

  Future<void> removeChannel(String channel) async =>
      await _awesomeNotifications.removeChannel(channel).then((value) =>
          _awesomeNotifications.cancelNotificationsByChannelKey(channel));

  Future<void> cancelBookingNotification(int bookingId) async =>
      await _awesomeNotifications.cancel(bookingId);
